...
│
├── Podman (rootless)
│   ├── mariadb          ← MariaDB 11 — Slurm job accounting database
...
- `slurmdbd` + MariaDB 11 provide full job accounting (`sacct`). Slurm 20.11+ dropped PostgreSQL support; MariaDB/MySQL is the only supported backend.
- All Slurm nodes share a single image (`localhost/cluster_slurm`) built from `cluster/Containerfile`
- `cgroup/v2` is disabled via `cgroup.conf` — it requires systemd D-Bus which is absent in rootless containers; `cgroup/v1` is used instead
...
This will:
1. Generate the munge authentication key (`cluster/conf/munge.key`)
...
│       ├── cgroup.conf            # Sets CgroupPlugin=cgroup/v1 (v2 requires systemd D-Bus)
│       ├── slurmdbd.conf          # Slurm accounting daemon → MariaDB
│       └── munge.key              # Shared munge auth key (generated by setup.sh, gitignored)
├── containers/
...
### cgroup/v2 and containers
Slurm 23.11 defaults to `cgroup/v2` which needs systemd D-Bus for scope creation — not available in rootless containers. `cluster/conf/cgroup.conf` sets `CgroupPlugin=cgroup/v1` to avoid this. Process tracking uses `proctrack/linuxproc` (set in `slurm.conf`).
